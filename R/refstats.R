...
#'
#' This function computes summary statistics for each referee, including total fouls called,
#' number of games officiated, fouls on a specific team, fouls on opponents, and the foul ratio.
#' It is built to work on raw play-by-play data where referees are listed in a single string column.
#'
...
#' @param foul_col Column name indicating whether the row is a foul (TRUE/FALSE).
#' @param game_id_col Column name for unique game identifiers.
#' @param home_col Column name indicating the home team.
#' @param action_team_col Column name indicating if the actioning team was "home" or "away".
#' @param team_name (Optional) The name of the team you want foul comparison stats for.
#'
#' @return A data frame summarizing referee stats, including fouls called, games worked,
#' fouls on the selected team and opponent, foul ratio, and fouls per game.
#' @export
#'
#' @examples
#' data(usu_data)
...
refstats <- function(data,
  ...
  if (!is.null(team_name)) {
    foul_details <- count_team_fouls(long_data, foul_col, home_col, action_team_col, team_name)
  } else {
...
#'
#' Helper function to split referees listed as a single string into individual rows.
#'
...
#'
#' Helper function to calculate how many fouls each referee called against a specified team
#' and against the opposing team, along with the foul ratio.
#'
...
#' @param action_team_col Column name indicating whether the action was by "home" or "away".
#' @param team_name Name of the team to analyze (e.g., "Utah State").
#'
#' @return A data frame with referee, fouls_on_team, fouls_on_opponent, and fouls_ratio.
